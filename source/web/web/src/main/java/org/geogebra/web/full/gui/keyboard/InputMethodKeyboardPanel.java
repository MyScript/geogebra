...
public class InputMethodKeyboardPanel extends FlowPanel
  ...
  private final Supplier<KeyboardListener> activeEditedItemSupplier;
   ...
   *        algebra item was last selected, independent of edit focus; used only
   *        to resolve the target when none is cached yet
   * @param activeEditedItemSupplier resolves a listener only for an item the
   *        user is genuinely editing right now (null otherwise, never falling
   *        back to a kernel selection or the input row); used to detect real
   *        row switches so a false positive from kernel-selection changes
   *        (e.g. a geo becoming selected as a side effect of committing a
   *        result) doesn't bounce the target to an unrelated row
  ...
  public InputMethodKeyboardPanel(String adapterUrl,
      ...
      Supplier<KeyboardListener> activeEditedItemSupplier,
      Runnable onSwitchToKeyboard) {
    this.adapterUrl = adapterUrl;
    this.settingsProvider = settingsProvider;
    this.lastSelectedItemSupplier = lastSelectedItemSupplier;
    this.activeEditedItemSupplier = activeEditedItemSupplier;
    ...
    StandardButton keyboardBtn = new StandardButton(
        KeyboardResources.INSTANCE.keyboard_show_material(), null, 24, 24);
  ...
  private KeyboardListener resolveCurrentTarget() {
    // only a genuinely edited row (real click/focus) counts as the user
    // switching rows; a kernel selection change caused by committing the
    // previous result is not navigation and must not bounce the target
    KeyboardListener activelyEdited = activeEditedItemSupplier.get();
    if (currentTarget != null && activelyEdited != null
        && !targetsSameRow(currentTarget, activelyEdited)) {
      // the user moved to a different algebra row (e.g. clicked it) since the
      // last result without going through Clear/Keyboard-switch: stop editing
      // the old row and start fresh on the newly active one
      finalizeCurrentTarget();
    }
    if (currentTarget == null) {
      ...
      if (currentTarget != null) {
        ...
        // an external input surface (e.g. an ink drawing canvas) can steal
        // native DOM focus from the row at any point during the session,
        // independent of and not synchronized with any replaceContent() call;
        // without this the row would auto-commit as a new geo on the first
        // such focus loss instead of staying open for the whole session
        setPreventBlur(currentTarget, true);
      }
    }
    return currentTarget;
  }

  private static boolean targetsSameRow(KeyboardListener a, KeyboardListener b) {
    if (!(a instanceof AlgebraMathFieldProcessing) || !(b instanceof AlgebraMathFieldProcessing)) {
      return a == b;
    }
    return ((AlgebraMathFieldProcessing) a).getAvInput()
        == ((AlgebraMathFieldProcessing) b).getAvInput();
  }

  private static void setPreventBlur(KeyboardListener target, boolean prevent) {
    if (target instanceof AlgebraMathFieldProcessing) {
      ((AlgebraMathFieldProcessing) target).setPreventBlur(prevent);
    }
  }
  ...
  private void finalizeCurrentTarget() {
    if (currentTarget != null) {
      // release the blur guard first so endEditing()'s blur actually commits
      setPreventBlur(currentTarget, false);
